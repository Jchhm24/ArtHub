...
export const BarSideSections = ({changeSection}) => {
    ...
    const changeSectionForm = (title, state) => {
        setTitulo(title)
    ...
    const {nombre, apellido, username, fechaNacimiento,email, contrasena, idRol} = formState

    return (
      <>  
        <div className="w-full p-5 flex flex-row gap-5">
          {/* BarSide */}
            <nav className="rounded-lg bg-nile-blue-950 p-2">
              <div className="flex flex-col gap-3">
                  ...
                  <button onClick={() => changeSectionForm('Datos personales', false)}
                  className={`${form1 ? 'SlideBar-button-active' : 'SlideBar-button'}`}>
                  ...
                  <button onClick={() => changeSectionForm('Datos de cuenta', true)}
                  className={`${form2 ? 'SlideBar-button-active' : 'SlideBar-button'}`}>
            ...
            <div className="flex flex-col gap-5 w-max">
                ...
                <form onSubmit={SubmitUser} className="rounded-lg bg-nile-blue-950 p-5 text-yellow-orange-300 flex flex-col justify-center gap-5">
                  {/* Datos personales */}
                    <section className={` ${form1 ? 'flex' : 'hidden'} flex-col gap-5`}>
                    ...
                    {/* Datos de cuenta */}
                    <section className={` ${form2 ? 'flex' : 'hidden'} flex-col items-center gap-5`}>
