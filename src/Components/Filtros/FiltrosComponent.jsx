...
export const FiltrosComponent = ({activeCheckboxes, setActiveCheckboxes}) => {
    ...
    return (
      <section className="flex flex-row gap-5 m-3 items-center justify-center">
        {/* Indicador de los filtrados activados */}
          <div className="flex flex-row items-center font-Red-Hat-Display font-medium bg-yellow-orange-400 rounded-full">
            <div className="bg-yellow-orange-300 rounded-full p-2">
              <svg className="fill-vulcan-950 rotate-90" xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24">
            ...
            <span className="pr-2 rounded-full text-center text-vulcan-950 font-Red-Hat-Display font-medium text-2xl">
                {count}
            </span>
          </div>
          ...
          <div className="flex flex-wrap gap-2">{
                ...
                ListCategorias.map((x => 
                    <div key={x.id} className="flex flex-row items-center font-Comfortaa font-medium">
                        <input type="checkbox" id={x.categoria} className="hidden peer" onChange={handleCheckboxChange}/>
                        <label htmlFor={x.categoria} className="cursor-pointer text-yellow-orange-300 py-1 px-2 rounded-full border-2 border-yellow-orange-300 peer-checked:text-vulcan-950 peer-checked:bg-yellow-orange-300">
                            {x.categoria}
                        </label>
                    </div>
                ))
